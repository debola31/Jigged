...
class PartValidationError(BaseModel):
    ...
    error_type values:
      ...
      - "invalid_cost": cost_per_unit (CSV column) is negative — execute
        routes this onto the part's tier row at quantity 1 when source='bought'.
...
PART_SCHEMA = {
    ...
    "cost_per_unit": {
        "type": "number",
        "required": False,
        "description": "Cost per primary unit (decimal, e.g., 12.50). For bought parts this is written onto the part's tier row at quantity 1; for made parts it is ignored (cost is computed live from routing + BOM).",
    },
    "reorder_point": {
        "type": "number",
        "required": False,
