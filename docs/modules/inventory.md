...
| **"Where is my o-ring?"** | Storage → the page search | [`StorageSearch`](../../components/inventory/locations/StorageSearch.tsx) matches **places and parts in one box**, grouped, **one row per part**. Picking a part opens [`PartPlacesDrawer`](../../components/inventory/locations/place/PartPlacesDrawer.tsx) — every place it is, with quantities and a total — and each row walks to that bin. |
