...
| Audit a place, or a whole cabinet | Storage → `Adjust` | The count worksheet, scoped to whatever was clicked — a container resolves to every leaf under it, which is how an audit physically happens: one cabinet, bin by bin. Was `Count or put away`; see §5.12. |
