...
| **The board draws nothing on a flat shop** (measured 2026-08-01, open question — #648) | `unitKind` has exactly ONE consumer (`boardChrome.tsx:278`), and for a node with no children the only thing `kind` changes is the rack left/right border (`:289-292`). The entire body — `CompartmentGrid`, `SectionShell`, the aisle branch — is behind `children.length > 0` (`:356`). So `floor`, `bench`, `outside`, `shelf` and `null` all render a byte-identical bordered rectangle. **118 of 121 shops are flat**, by the same research §5.5 uses to cut drag-to-reparent — so for almost every real shop the board is a grid of labels, and the argument that beat the tabular list ("at 12–18 places the board wins") rests on a payload it isn't delivering. Separately: the top-level palette is unreachable. `subdividing = parentId !== null` is always true because `openTopLevelBuilder` was deleted in faf1e0a, so `STORAGE_TYPES` (10 cards) and the title `'Build storage visually'` never render — the founder was literally right that there is no visual builder at the front door. **The question is therefore not "board or table" but "what should a childless tile draw".** Not resolved here; the elaborate revival specced against it was withdrawn (its own `containerName` machinery broke top-level numbering and three existing tests). |
