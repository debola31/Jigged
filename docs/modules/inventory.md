...
tab** (where every stock write happens), and **Storage** (where it lives). All three are core for
every tenant: the `inventory_locations` flag that used to gate Storage was retired Aug 2026.

| I want to… | Where | How |
|---|---|---|
| Decide we stock a part at all | `/parts/{id}` → workspace | Primary unit, reorder point. ⚠ *This row used to say the tab appears only once `Stocked` is on. There is no such flag since `is_stocked` was dropped ([#778]) — every part is stockable, so the **Storage** tab is always there.* |
| See what we have | `/inventory/locations` (**Storage**) | Parts carries no quantities: it is the item master. ⚠ **There is no shop-wide shortage lens any more** — it was the Parts stock filter, removed with `is_stocked`, and Storage has not grown a replacement. `JobPartMaterialsCard`'s *"N short"* chip is now plain text for that reason. |
...
| **Add / remove / adjust stock** | `/parts/{id}` → Storage tab | The four buttons. Until receiving (J6) exists **this is how stock gets in**. |
...
| See where one part lives | `/parts/{id}` → Storage tab | Per-location balances with full paths. |
...
| **Count one part at one place** | `/parts/{id}` → Storage tab → the icon on a balance row | Offered on **every** row including zeros. |
...
> Verified against the code on `main` as of 2026-07-27. Current state; §4 onward is target. Columns: [`types/database.ts`](../../types/database.ts).
...
**Nothing here is behind a feature flag.** `inventory_locations` was retired Aug 2026 — the Locations entry, the route, the operator Inventory tab and the auto-enrol trigger are core for every tenant, and [`20260824221219`](../../supabase/migrations/20260824221219_retire_three_feature_flags.sql) strips the dead key out of `companies.settings`. **Withdrawn:** this section called the flag *"opt-in, default off for every tenant"* — wrong from the `is_stocked` removal onward, which flipped it opt-out, and contradicted by §5.12 in this same file from that day on. A registry list copied into prose is exactly the claim no build falsifies, so there is no longer one here: [`lib/featureFlags.ts`](../../lib/featureFlags.ts) is the registry. There is **no `inventory_transactions` flag** either, despite #550 and earlier revisions citing one.
...
> **The bug the naive version would have armed, reproduced before writing the migration.** `trg_auto_track_stocked_part` was `AFTER INSERT OR UPDATE **OF is_stocked**`, and `updatePart` sent `is_stocked` on every save — so it re-ran when someone merely renamed a part. (⚠ Both the column and that trigger are gone: the replacement, `trg_seed_new_part_balance`, is `AFTER INSERT` only, so this hazard is now unrepresentable rather than defended. The `NOT EXISTS` guard is kept anyway and still pinned.) Its `ON CONFLICT DO NOTHING` was being satisfied by nothing except the part's own leftover zero row. Delete the residue without fixing the trigger and the next rename re-inserts the part's whole quantity at `Unassigned` on top of the stock already on its shelf: **580 → 1160**, silently, with no ledger row to recover from. Hence the migration fixes every producer *before* it deletes a single row, and [`test_balance_rows_are_holdings.py`](../../api/tests/integration/test_balance_rows_are_holdings.py) pins it — the constraint cannot, because both the row and the rollup stay internally consistent, just consistently wrong.
...
**Flag gating: settled 2026-09-09 — there will be no `inventory_transactions` flag.** Owner decision, closing the last of the #550 residue ([#668](https://github.com/debola31/Jigged/issues/668)): the reorder-point / low-stock status is **core for every tenant** and renders unconditionally, as it always has. The flag was never registered in [`lib/featureFlags.ts`](../../lib/featureFlags.ts) and never will be, so the plan to hide FR-2 behind it is **withdrawn**, not deferred — this doc no longer proposes gating that §3's *Archive · dead code* already says does not exist. **Still open, and a separate question:** PRD **FR-2 is a `Must`** (dashboard *plus* email) while this doc calls it a `Should`, partly delivered. ([`ai-insights.md`](ai-insights.md) no longer claims the badge as built — that line went with the badge.) FR-2 now rests entirely on the shortage lens, with no notification of any kind.
...
Two engines (§3) — one atomic, one a client-side race, chosen by a boolean — is the deepest debt. **Collapse onto the RPCs** (PR #446: a part left at `Unassigned` behaves exactly like a global-quantity item, so the location path is a strict superset); blocker was that the flag would then become the data model with an `Unassigned`-only default, so it needed its own PR — moot since the flag was retired Aug 2026. **Re-dated after J7 (2026-07-28):** J7 made the non-atomic path the highest-frequency write, run concurrently, so the exposure is **lost update**, not just crash-between-round-trips — two operators each take 5 from 100, stock lands at 95 not 90, with two correct-looking ledger rows. **Withdrawn:** *Contour is safe because its flag is on; a flag-off shop with two operators is not* — there is no flag-off shop. 20260802015837 deleted the four aggregate writers outright (`parts.quantity` is a trigger rollup that now refuses a direct write), so the engine choice was gone before the flag was, and the proposed `deplete_part_stock` SECURITY DEFINER RPC — a mirror of `deplete_stock_at_location` meant to close the race without the collapse — was never needed. **⚠ Stale, and knowingly left:** the rest of this section and §3's two-engines table still describe a boolean-selected aggregate path that no longer exists.
...
| ~~**`Count all parts`** lands here~~ → **removed 2026-08-10** | `/inventory` folded into Parts, leaving no sibling page — so it landed on Storage by elimination rather than because anyone audits a whole shop that way. `Adjust` on a unit replaced it with the scope people actually use, and `Count Inventory` on the Parts toolbar then held the shop-wide door — until that button went with the stock columns, leaving Storage and the part's Inventory tab as the doors. |
...
| The cert field is on every SINGLE-part receipt, including the side rail's | `PlaceStockActionForm` is two forms in one component: restricted to one part it is the side rail's per-place Add and follows the dialogs' rule, and unrestricted it is a batch of lines each with its own heat, where one file field could not say which line it belonged to — that mode keeps its after-the-batch panel. Missing this on the first pass left the cert absent from the surface an Inventory row opens into, which is the likeliest place to receive against a heat |
...
| Both arms of the tier rule, floor included | Highest break ≤ quantity, **floored to the lowest break** when the shop holds fewer than the smallest one — mirroring `part_rollup_at_qty`'s bought arm and `resolveTier`. Taking only the first arm would report every part held in small quantity as uncosted, manufacturing gaps out of ordinary data. The view re-implements the rule rather than calling the function per row, so parity is pinned by `test_inventory_on_hand_cost.py` — two implementations of one rule drift silently, and the table would still render, still add up, and be wrong by a tier |
...
| Area | Enforced by |
|---|---|
| Delete-as-archive, single and bulk | `__tests__/utils/partsAccess.test.ts` |
...
| A certificate really reaches the bucket — the row in `lot_certificates` and the object in `attachments` land together at the `{companyId}/lots/{lotId}/…` path its RLS gates on. The mocked tests deliberately cut this seam, and a wrong storage prefix would leave every one of them green while every upload 403'd. Plus: skipping the cert still records the stock, and exactly ONE control offers to attach it | `e2e/storage-certificates.spec.ts` (3 its, serial) |
| The Inventory tab is what you land on, a `?unit=` deep link still reaches the board, per-place grain gives two rows one shared unit cost, and the footer follows the filter | `e2e/storage-inventory.spec.ts` (5 its) |
| The view's cost equals `part_rollup_at_qty`, **floor arm included** — the case no seed data reaches, because every seeded part's smallest break is 1, so it is constructed. Plus made-vs-missing-tier as distinct gaps, the tier chosen at the part total rather than per shelf, and an archived part leaving the table | `api/tests/integration/test_inventory_on_hand_cost.py` (7 its) |
