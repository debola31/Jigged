...
| **Move stock at a place** | Storage → click a cell → drawer → `Add` · `Remove` · `Move` | One part at a place, in a form that fixes the *place* and picks the *part* — the inverse of the part page, and the information you actually have standing at a cabinet. [`PlaceStockActionForm`](../../components/inventory/locations/place/PlaceStockActionForm.tsx), **new 2026-08-10**: before it, Storage could not put anything into a bin at all. |
...
| Audit a whole cabinet | Storage → pick a unit → `Bulk Adjust` | The `Bulk Adjust {unit}` worksheet, which resolves a container to every leaf under it — one cabinet, bin by bin, with search, paging and per-line commit reporting. Was `Count or put away`, then `Adjust`; see §5.12. |
...
| **"Where is my o-ring?"** | Storage → the page search | [`StorageSearch`](../../components/inventory/locations/StorageSearch.tsx) matches **places and parts in one box**, grouped. A part row names the shelf and the amount (`Cabinet 3 › Shelf A · 828 ea`) and picking it selects that unit AND opens that bin — the cabinet is not the answer, the shelf is. A part in three bins is three rows: you are not learning how many you own, you are learning which shelf to walk to. |
