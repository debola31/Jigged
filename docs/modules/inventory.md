...
**The row rule, and the two simpler rules that are both wrong.** One row per place holding `quantity > 0`, plus exactly one row at the system bucket when a part holds stock nowhere. Emitting a row per balance *row* fills the sheet with the zero residue `transfer_stock` and `bulk_put_away` leave behind forever — a live absolute write target on a shelf the part has left, which makes a count worse than not counting. Filtering to `> 0` alone deletes the opening count: `trg_auto_track_stocked_part` seeds every stocked part at `Unassigned` with 0, so a first-time count would find its whole catalogue missing — and *"the system says zero and I am holding twelve"* is the count that matters most. On the seed's 14 stocked parts the three rules give 18, 11 and 15 rows; the middle one drops 4 parts.
