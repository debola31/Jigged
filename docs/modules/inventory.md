...
| Flag off (`inventory_locations`) | **Storage** nav hides. ⚠ The flag is **opt-out (default on)** as of the `is_stocked` removal, so this is now a deliberate per-tenant kill-switch rather than the common case. It had to flip: `Count Inventory` lived on the Parts toolbar precisely because gating it on this flag once took counting away, and that button is gone — leaving Storage as the door, which every tenant must therefore have. |
