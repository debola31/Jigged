...
| **Move stock at a place** | Storage → pick a place → `Add` · `Remove` · `Move` | One part at a place, in a dialog that fixes the *place* and picks the *part* — the inverse of the part page, and the information you actually have standing at a cabinet. [`PlaceStockActionModal`](../../components/inventory/locations/PlaceStockActionModal.tsx), **new 2026-08-10**: before it, Storage could not put anything into a bin at all. |
| Audit ONE place | Storage → pick a place → `Adjust` | [`PlaceAdjustModal`](../../components/inventory/locations/PlaceAdjustModal.tsx): everything at that place, a number per row, variance as you type. **A blank is not a zero** — untouched rows keep their balance. |
| Audit a whole cabinet | Storage → pick a unit → `Adjust` | The count worksheet, which resolves a container to every leaf under it — one cabinet, bin by bin, with search, paging and per-line commit reporting. Was `Count or put away`; see §5.12. |
...
| Count the whole shop | `/parts` → `Count Inventory` | One door. Storage's `Count all parts` went 2026-08-10 — nobody audits a shop as one list of every stocked part, and Storage's own scope is now the container you are looking at. |
...
| 3 | Wizard survives only as **Subdivide** an existing unit | Reaches `VisualLocationBuilder`'s dormant `parentId`. Tested traps: `STORAGE_TYPES` level 0 *is* the container (→ `Cabinet 3 › Cabinet 1 › Row 1`), hence separate `SUBDIVISION_TYPES`; re-subdividing must continue numbering (Row 4–6). That last one used to be load-bearing for *atomicity* — a collision mid-run left a partial tree — and since 2026-08-10 it is not: `create_location_tree` is one transaction. Keep it anyway, because continuing the numbering is what the operator meant. |
...
**Numbers pinned:** board load = **2 requests exactly**, at 40 and 400 locations; put-away = **one** RPC at any count, DB-capped at **1,000** parts; repeat subdivide continues **Row 4–6**; **7** foreign codes rejected; `referencedTable` must be the embed *alias* or paging fails at runtime with `PGRST108`; `Unassigned` is unrenameable — the stock RPCs resolve it by literal name.
