...
| payment_terms | Text | Yes | Payment terms shown on the quote (preset or custom free text), e.g. `Prepay`, `Net 30`, `2/10 Net 30`. A single free-text string (no enum); the form offers a pick-or-add combobox (presets + the shop's saved custom terms, stored on `companies.settings.custom_payment_terms`, with an "Add New" action to add + save more). Required at the form level — every quote carries payment terms |
...
- Payment terms — a **pick-or-add combobox** (MUI `Autocomplete`, **pick-only** — you don't free-type into it). It lists a short, **flat** set of terms with **no group subheaders**: the shop's saved custom terms first (each with a **✕** to remove it), then the standard presets (**Due on Receipt · Net 15 · Net 30 · Net 60 · 2/10 Net 30 · 50% Deposit / Balance Net 30 · Prepay · Cash on Delivery** — QuickBooks' built-in terms plus the deposit / prepay / COD and early-pay-discount terms shops use). A highlighted **`＋ Add New`** action is pinned as the **last** row (accent-coloured + semibold, above a divider). Choosing it closes the list and reveals an **inline field just below the picker** ("New payment term" + **Add** / **Cancel**) — the field lives below rather than inside the dropdown because a menu closes on selection, so an in-menu input can't reliably hold focus. Submitting the field (Add or Enter) selects the term on this quote **and** saves it to the company (`companies.settings.custom_payment_terms`, a string array — no migration, it rides the existing settings jsonb) so it reappears — at the top, removable — for reuse. Presets are **not** removable (only shop-added terms carry the ✕). Presets live in `PAYMENT_TERM_PRESETS` ([types/quote.ts](../../types/quote.ts)); saved terms are read/written via `getCustomPaymentTerms` / `addCustomPaymentTerm` / `removeCustomPaymentTerm` ([utils/companyAccess.ts](../../utils/companyAccess.ts)) with the pure reader `readCustomPaymentTerms` ([lib/companyDefaults.ts](../../lib/companyDefaults.ts)). The stored value on the quote stays a single free-text `payment_terms` string (no enum). Required — the form blocks submission until a value is entered.
