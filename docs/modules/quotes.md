...
- Payment terms — a **type-or-pick combobox** (MUI `Autocomplete`, free-solo). It offers a short flat preset list (**Due on Receipt · Net 15 · Net 30 · Net 60 · 2/10 Net 30 · 50% Deposit / Balance Net 30 · Prepay · Cash on Delivery** — QuickBooks' built-in terms plus the deposit / prepay / COD and early-pay-discount terms shops use) under a **"Standard terms"** group, and the shop's own saved terms under a **"Your saved terms"** group. An always-visible **`＋ Type to add your own term`** hint sits at the top of the open list (a disabled cue) so custom entry is discoverable *before* the user types. Typing a term that isn't already listed turns that row into an actionable inline **`Add "<typed>"`**; choosing it commits the term **and** saves it to the company (`companies.settings.custom_payment_terms`, a string array — no migration, it rides the existing settings jsonb) so it reappears for reuse — each saved term carries a **✕** to remove it. Persistent helper text ("Choose a standard term or type your own") makes custom entry discoverable (a placeholder would vanish on focus — see NN/g). Presets live in `PAYMENT_TERM_PRESETS` ([types/quote.ts](../../types/quote.ts)); saved terms are read/written via `getCustomPaymentTerms` / `addCustomPaymentTerm` / `removeCustomPaymentTerm` ([utils/companyAccess.ts](../../utils/companyAccess.ts)) with the pure reader `readCustomPaymentTerms` ([lib/companyDefaults.ts](../../lib/companyDefaults.ts)). The stored value on the quote stays a single free-text `payment_terms` string (no enum). Required — the form blocks submission until a value is entered.
