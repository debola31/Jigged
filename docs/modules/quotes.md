...
| payment_terms | Text | Yes | Payment terms shown on the quote (preset or custom free text), e.g. `Prepay`, `Net 30`, `2/10 Net 30`. A single free-text string (no enum); the form offers a type-or-pick combobox (presets + the shop's saved custom terms, stored on `companies.settings.custom_payment_terms`). Required at the form level — every quote carries payment terms |
...
- Payment terms — a **type-or-pick combobox** (MUI `Autocomplete`, free-solo). It lists a short, **flat** set of terms with **no group subheaders**: the shop's saved custom terms first (each with a **✕** to remove it), then the standard presets (**Due on Receipt · Net 15 · Net 30 · Net 60 · 2/10 Net 30 · 50% Deposit / Balance Net 30 · Prepay · Cash on Delivery** — QuickBooks' built-in terms plus the deposit / prepay / COD and early-pay-discount terms shops use). A single **add** row sits at the **top** (divider below it) so custom entry is visible the instant the list opens, styled **prominently** (accent-coloured + semibold, like a section label — not a faint greyed row): a non-selectable **`＋ Type to add your own term`** cue when the field is empty, becoming an actionable **`＋ Add "<typed>"`** once the user types a term that isn't already listed. Because the top row is a *disabled* hint when empty, the first *selectable* option is still the most common term, so frequency-first ordering holds. Choosing it commits the term **and** saves it to the company (`companies.settings.custom_payment_terms`, a string array — no migration, it rides the existing settings jsonb) so it reappears for reuse. Persistent helper text ("Choose a standard term or type your own") reinforces that custom entry is possible (a placeholder would vanish on focus — see NN/g). Presets live in `PAYMENT_TERM_PRESETS` ([types/quote.ts](../../types/quote.ts)); saved terms are read/written via `getCustomPaymentTerms` / `addCustomPaymentTerm` / `removeCustomPaymentTerm` ([utils/companyAccess.ts](../../utils/companyAccess.ts)) with the pure reader `readCustomPaymentTerms` ([lib/companyDefaults.ts](../../lib/companyDefaults.ts)). The stored value on the quote stays a single free-text `payment_terms` string (no enum). Required — the form blocks submission until a value is entered.
...
5. Insert **one** `jobs` row carrying `quote_id`, `customer_id`, `due_date`, `customer_po_number`, `production_status = 'not_started'`. Job number via `nextQuoteJobNumber`: the mirror `Q-NNNN → J-NNNN` when free (the first, common case), otherwise the next free suffix `J-NNNN-2`, `J-NNNN-3`, … (a later PO on the same quote, or a mirror held by a since-archived job).
