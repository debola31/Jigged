...
| FR-3b | Direct Job Creation from Purchase Order | When a customer sends a PO with no prior quote, the salesperson can create a job directly via **New Job from PO** on the jobs list: pick the customer, enter the PO #, add one or more **existing** parts (each with a quantity and the agreed unit price), optionally attach the PO PDF, and accept. No quote is involved (`quote_id` null; job number `PO-J-NNNN`). v1 supports existing parts that already have a routing. | Must | Given a customer PO for existing parts, when the salesperson completes "Accept Purchase Order", then a job is created with the PO #, the agreed price on each part, cloned routing operations, and the optional PO PDF attached. |
...
| FR-15 | QuickBooks Integration | Invoices can be synced to QuickBooks Online via OAuth connection. Sync creates matching invoice in QuickBooks with customer and line item mapping. Invoicing is **job-keyed** and initiated from the job detail page (not the quote), so quote-sourced and PO-sourced jobs invoice the same way; price is read from `job_parts.unit_price`. | Should | Given a Jigged invoice, when bookkeeper clicks Sync to QuickBooks, then invoice appears in QuickBooks with correct customer. |
