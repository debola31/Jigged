...
| FR-5 | Operator Station Sign-In | Email/password on the operator's own phone (session persists, so effectively one-time per device), then pick the work centre from the in-app selector. Remembered per device, changeable from the header. **No PIN, no badge.** *(Posted station-QR placards were removed in July 2026 — never deployed to a floor)* | Must | **Built** |
