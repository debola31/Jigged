...
| The first question after an idle spell fails as offline; the next one works | Cold load plus the ~13K-token prefill exceeded `request_timeout_s` (`worker/config.py`), and a timeout classifies as `ai_offline`. Measured 2026-09-07 on the 48 GB M4 Max: the cold first call ran past 120 s, the warm median was 25 s, so the constant is 240 s. Check `OLLAMA_KEEP_ALIVE=-1` and pre-warm after start; if a warm box still runs past ~3 minutes, raise the constant — not a new env knob. |
