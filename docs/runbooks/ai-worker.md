...
| The first question after an idle spell fails as offline; the next one works | Cold load plus the ~13K-token prefill exceeded `request_timeout_s` (120 s, `worker/config.py`), and a timeout classifies as `ai_offline`. Check `OLLAMA_KEEP_ALIVE=-1` and pre-warm after start; if a warm box still runs past ~90 s, raise that constant — not a new env knob. |
