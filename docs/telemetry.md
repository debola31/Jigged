...
| `report generated` | A one-page summary job settles, once per job. `phase` is the local box's reliability for a multi-call job the way `ai job settled` is for a question; the counts are the shape of what the model composed and what the gate removed (`dropped_count` is chart blocks the chat gate refused); `tool_call_count` is how many queries it took, which is how long the report held the box's single slot. Never the request, the title or a figure | `phase`, `error_kind`, `kpi_count`, `block_count`, `chart_count`, `table_count`, `has_text_block`, `dropped_count`, `tool_call_count` | [InsightsChat.tsx](../components/insights/InsightsChat.tsx) |
