...
| `quote converted to job` | A quote pass is accepted and becomes jobs — **one job per checked part**, so a single click can create several. Still one event per action rather than per job: firing per job would inflate the denominator and make "how many quotes were won" unanswerable, which is the question this event exists for, and `part_count` already carries the fan-out width. `failed_count` is what the fan-out adds — a pass is no longer all-or-nothing, and a non-zero value means parts stayed on the quote after a click that looked like it converted them | `quote_id`, `part_count`, `failed_count` | [ConvertToJobModal.tsx](../components/quotes/ConvertToJobModal.tsx) |
| `job created from purchase order` | A job is created directly from a PO | `part_count`, `total_value` | [AcceptPurchaseOrderModal.tsx](../components/jobs/AcceptPurchaseOrderModal.tsx) |
...
| `vendor service created` | An outside process is added to a vendor. **`has_price` is the number this feature exists to move**: 89% of outside routing steps carried no price at the split, and a service is where a shop can set one once instead of per step. If services keep arriving unpriced, naming the process solved the smaller half of the problem | `has_price` | [VendorServicesCard.tsx](../components/vendors/VendorServicesCard.tsx) |
...
| `stock updated` | Stock is adjusted. `surface` says which UI — `office` (the side rail on Storage → Inventory; the part page's own Storage tab until 2026-09-09), `storage` (the four verbs on a place, Storage tab), `operator` (the four verbs on a part already in the bin), `operator_receive` (stocking a part into a bin it was not in yet); `location_id` is absent only on `office`, which picks the place inside the dialog. `heat_captured` (2026-09-04) says whether a mill heat number was typed on the movement — a boolean, never the heat itself, which is the customer's business data; it is how adoption of the optional field is measured across shops | `surface`, `action`, `part_id`, `quantity`, `unit`, `location_id`, `heat_captured` | [PlaceStockActionForm.tsx](../components/inventory/locations/place/PlaceStockActionForm.tsx) · [OperatorLocationActionModal.tsx](../components/operator/OperatorLocationActionModal.tsx) · [OperatorReceivePartModal.tsx](../components/operator/OperatorReceivePartModal.tsx) |
| `stock movement annotated` | The office corrects a ledger row from the part's history — the notes and/or the heat number, the only two columns a movement lets you change after the fact. Two booleans say which moved; neither value is sent. Exists because a corrected heat is the one path that can make a packing slip and the ledger disagree (the slip keeps its frozen copy), so how often it happens is worth knowing | `notes_changed`, `heat_changed` | [HistoryTab.tsx](../components/parts/workspace/tabs/HistoryTab.tsx) |
...
| `operation completed` | An operation is completed. **No longer operator-only** — as of 2026-08-28 the office's Complete button fires it too, so `surface` — `operator` or `office` — is now present and is the property to split on. That split is the question the change was for: how much production is recorded by the person who did it versus keyed in afterwards by someone who was not there, because only the first kind ever carries a duration. `discarded_running_timers` is `office`-only and is normally 0 — non-zero means the office completed a step somebody was standing at and that operator's measured minutes were thrown away, which is the cost of refusing to fabricate a finish time and needs a number rather than an anecdote | `surface`, `job_operation_id`, `quantity_good`, `is_partial`, `discarded_running_timers` | [operations/page.tsx](../app/operator/[companyId]/jobs/[jobId]/parts/[jobPartId]/operations/[jobOperationId]/page.tsx) · [OperationsPanel.tsx](../components/jobs/OperationsPanel.tsx) |
| `operation completion conflicted` | Two people reached for the same step and the second one's submit was refused — first write wins, and nothing was recorded. **This is the number that says whether the race is real**: the guard is a re-read immediately before the insert, which closes the minutes-long stale-dialog window and not a sub-second double-submit, and if this fires often enough to matter the answer is to make the write atomic in an RPC rather than to widen the check. `surface` says who lost, which is the more interesting half — the office losing to the floor is the system working, the floor losing to the office is a step being closed out from under somebody | `surface` | [operations/page.tsx](../app/operator/[companyId]/jobs/[jobId]/parts/[jobPartId]/operations/[jobOperationId]/page.tsx) · [OperationsPanel.tsx](../components/jobs/OperationsPanel.tsx) |
...
| `time interval closed` | An operator records what they finished, closing the interval. There is no `close_reason` property because a close carries no choice: an explicit close is always a completion, and `switched` is written server-side by the chain rather than by a tap. (Until 2026-08-26 the reason given here was that a completion is the *only* deliberate end; that stopped being true when `time interval cancelled` shipped, but a cancel is a separate event rather than a variant of this one, so the property is still absent for the same practical reason.) `was_adjusted` is the signal that survives — a high rate means the clock is not being started when the work is | `was_adjusted` | [OperatorIntervalContext.tsx](../components/operator/OperatorIntervalContext.tsx) |
| `running timer discarded` | The office discards a timer from the dashboard's unfinished-work band — the sibling of `time interval cancelled`, which is the operator discarding their own. **This is the forgotten-stop rate**, and it is the number the staleness threshold should be tuned against: since 2026-09-07 that threshold is three times the step's estimate, floored at an hour and capped at six ([`longRunningThresholdMinutes`](../lib/duration.ts)), so a rise here after that change means the factor is too loose. Carries no duration and no operator: `get_open_intervals` returns neither, so there is nothing here that could resolve a machine to a person, and adding one would be the surveillance guardrail's exact failure mode | `surface` | [UnfinishedWorkBand.tsx](../components/dashboard/UnfinishedWorkBand.tsx) |
| `time interval cancelled` | An operator discards a running timer with `Cancel activity` — they started a step and produced nothing, so the interval is voided rather than closed. **This is the number that says whether the button was needed**: before it existed the only way to stop such a clock was to record a quantity you had not made and then undo it, which the E2E suite did deliberately and a person did by hand in production twice. `elapsed_bucket` (`under_1m`, `1m_15m`, `15m_1h`, `1h_6h`, `over_6h`) separates a mis-tap from a walked-away — coarse and bucketed on purpose, because a raw per-person duration is what the surveillance guardrail is about and it answers no question the bucket does not | `elapsed_bucket` | [OperatorIntervalContext.tsx](../components/operator/OperatorIntervalContext.tsx) |
| `time interval paused` | An operator taps Pause — they are stopping and mean to come back, so the span is CLOSED and its minutes kept, unlike `time interval cancelled` which voids. Shipped 2026-09-07 with the reversal of the "No pause / resume" non-goal, and **this is the number that says whether the reversal was right**: the recorded objection was that a paused state is one more thing to remember to undo, so read it against how many of these are still unresumed on the office card a day later. `elapsed_bucket` (`under_1m`, `1m_15m`, `15m_1h`, `1h_6h`, `over_6h`) separates a short break from an end-of-shift — coarse and bucketed for the same reason as its sibling, because a raw per-person duration is what the surveillance guardrail is about | `elapsed_bucket` | [OperatorIntervalContext.tsx](../components/operator/OperatorIntervalContext.tsx) |
| `time interval resumed` | An operator picks a paused step back up. The underlying write is `start_operation_interval`, exactly as a cold start — this is a separate event rather than a property on `time interval started` because the two answer different questions, and folding them together would make `had_open_interval` mean two things. `paused_bucket` is how long it sat, on the same scale, and **the pair (paused, resumed) is the feature's actual measure**: pauses that never resume are the failure mode | `paused_bucket` | [OperatorIntervalContext.tsx](../components/operator/OperatorIntervalContext.tsx) |
...
| `note posted` | Somebody saves a note. `surface` says from where: `operator_step` (a step), `operator_job` (the whole-job traveler, where no step is chosen), `operator_machine` (the machine log), or `office_job` (the job page's activity rail — the one office composer, text-only, which is why its `photo_count` and `video_count` are always 0). **This closes the gap named below**: the notes-and-photos loop had no event at all and was measured by an autocapture text-match on the Post button, which is also what blocked `mask_all_text`. Deliberately shape-only — `has_text` and not the text, because an operator note is the customer's business data. `video_count` is the number the in-app recorder shipped to find: whether anyone reaches for video at all | `surface`, `has_text`, `photo_count`, `video_count` | [useNoteCapture.ts](../hooks/useNoteCapture.ts) · [JobActivityComposer.tsx](../components/jobs/activity/JobActivityComposer.tsx) |
...
| `job list searched` | An operator settles on a query in the jobs-list find field — once per distinct query, on the same debounce that writes the URL, never per keystroke. **This is the number that says whether the field was needed at all.** The All Stations lens was built to answer "where is job #123?" and only ever answered the other half of its brief, so this is a bet that finding a named job is a real floor task rather than an office habit projected onto a phone; if nobody types, it is dead weight and can go, which is the same reasoning that instrumented the notes-sheet scope toggle. `has_results` and not a match count — the shape of the answer is what tells a fruitless search from a successful one, and a tally on an operator surface has to justify itself against the [guardrail](modules/operator-view.md#surveillance-guardrail-non-negotiable) every time. No query text: a job number, part or customer name is the shop's business data | `surface`, `scope`, `has_results` | [jobs/page.tsx](../app/operator/[companyId]/jobs/page.tsx) |
| `activity rail toggled` | Someone opens or closes the job page's activity rail. **This is the number that says whether open-by-default was right.** The rail is docked and open on arrival because discoverability of the feed — and of its composer — is the reason it is a rail rather than an on-demand drawer; a high close rate with few reopens means the office is reclaiming the width and the drawer was the better shape after all. `is_wide` separates the docked column from the narrow-screen overlay, where closing is simply how you get back to the page and means nothing about the shape. No open-duration and no scroll depth: the toggle is the decision, and the rest would be watching someone read | `surface`, `open`, `is_wide` | [JobActivityRail.tsx](../components/jobs/activity/JobActivityRail.tsx) |
| `activity step filtered` | The note count on a step card is pressed, narrowing the rail to that step — the affordance that replaced the card's expand chevron when completion history, vendor slips and notes moved into the feed. **If this never fires, the badge is decoration** and the count should become plain text or go. `cleared` true is the press that returns to the whole job, which is the other half of whether a filtered view is a place people are willing to go. `note_count` is a count of rows, never their content | `surface`, `note_count`, `cleared` | [OperationCard.tsx](../components/jobs/OperationCard.tsx) · [JobActivityRail.tsx](../components/jobs/activity/JobActivityRail.tsx) |
| `completion undone` | The office takes back a recorded completion from the activity rail. It shipped on the operation card's expand and was **never instrumented**; this is the write moving, which is the moment to close that gap. `capture_source` is the split worth having — the office undoing a row it keyed in itself is a typo, and the office undoing an `operator` row is the office overruling the floor. They are one button and two different things happening in the shop. (Named for the ACTION, not the column: the row is stamped `voided_at`, but void is document language — it belongs to packing slips and invoices, and the step card has always called this Undo.) | `surface`, `capture_source` | [JobActivityRail.tsx](../components/jobs/activity/JobActivityRail.tsx) |
...
| `ai job enqueued` | Someone asks the AI a question and the job row is created. The denominator for everything below, and `executor` is the rollout signal: it says whether the shop's own box or a hosted model was asked. `turn_index` is how many answered exchanges the conversation already held (0 = a fresh thread) — the multi-turn adoption signal, and the only thing about the thread that leaves the row. Whether the answer became a one-page report is not known here — the model decides from the words, mid-job — so that is `report generated`'s to say. `from_example` and `from_suggestion` are the two ways a question can arrive without being typed: an example chip on the empty page, or one of the model's own follow-ups under the previous answer. They are separate booleans rather than one source enum because `from_example` predates the follow-ups and its history stays comparable — and because the check reads literal property keys | `feature`, `executor`, `from_example`, `from_suggestion`, `turn_index`, `question_length_bucket` | [InsightsChat.tsx](../components/insights/InsightsChat.tsx) |
| `ai job refused` | A question is turned away at the door and never becomes a job row, so `ai job settled` cannot see it. **This event exists because those refusals were invisible in both tools** — the enqueue `catch` captured nothing, and Sentry is deliberately skipped for every one of these statuses — which is how "does the shop ever ask and find the box off" stayed unanswerable through the pilot. `reason` is what turned it away: `offline` (503, the AI unreachable), `rate_limited` (429, the shop's own cap), `disabled` (403, its own kill-switch), `busy` (409, one question at a time), `thread_missing` (404, a thread archived out from under the tab). **`offline` is a rate here rather than a Sentry incident, and that is the considered choice**: a box that is asleep has no code fix, so paging on it would fill the queue with the non-actionable issues `ignoreErrors` exists to keep out — the failure mode that left this project with 55 stale ones. What was missing was never the alert, it was how OFTEN. Sentry is unchanged: the reason list is exactly the statuses it already skipped, so anything unmapped still pages. **This is the sibling of `phase: offline` on `ai job settled`** — that one is the box dying mid-job, this one is the box being off before the job exists, and only the two together give the real unavailability rate | `feature`, `reason`, `turn_index`, `from_example`, `from_suggestion`, `question_length_bucket` | [InsightsChat.tsx](../components/insights/InsightsChat.tsx) |
| `ai job settled` | That job reaches a terminal state, once per job. `phase` is what a local-inference rollout is judged on — the ratio of `offline` to `done` IS the question "is the box reliable enough", and nothing else answers it. `error_kind` separates a box being asleep from a model failing. `off_topic` is the templated "I can only answer questions about this shop" refusal: its rate is the one number that decides whether an input gate is ever worth building (the plan defers one behind 5 % over 30 days), and it says nothing about what was asked. `grounding_corrected` is the model stating figures its queries did not produce and being sent back to the tool once (the 25-turn live thread did it on six turns of twenty-four): its rate is how often a conversation tempts the model to answer from memory. `follow_up_count` is how many next questions the model offered (0 is normal and expected — a refusal or a dead end offers none); against `from_suggestion` on the next enqueue it is the whole case for whether suggestions earn the tokens they cost | `feature`, `phase`, `executor`, `model`, `error_kind`, `has_chart`, `tool_call_count`, `off_topic`, `grounding_corrected`, `follow_up_count`, `duration_bucket`, `tool_names` | [InsightsChat.tsx](../components/insights/InsightsChat.tsx) |
| `report generated` | A one-page summary job settles, once per job — asked through the report door or, since 2026-09-08, chosen by the model from the words of a question (the job row's `kind` reads `report` either way). `phase` is the local box's reliability for a multi-call job the way `ai job settled` is for a question; the counts are the shape of what the model composed and what the gate removed (`dropped_count` is chart blocks the chat gate refused); `tool_call_count` is how many queries it took, which is how long the report held the box's single slot. Never the request, the title or a figure | `phase`, `error_kind`, `kpi_count`, `block_count`, `chart_count`, `table_count`, `has_text_block`, `dropped_count`, `tool_call_count`, `duration_bucket` | [InsightsChat.tsx](../components/insights/InsightsChat.tsx) |
| `report exported` | Someone downloads a summary PDF from the preview. **This is the number that says whether a page ever leaves the screen** — a report that is generated and never downloaded was a curiosity, one that is downloaded was carried into a meeting. Shape only | `block_count`, `chart_count` | [ReportPreviewDialog.tsx](../components/insights/ReportPreviewDialog.tsx) |
| `insights history opened` | The History rail opens, or changes tab while open. Whether anyone ever opens History is the number that says whether the rail earns its button; `tab` is which of the three lists they went to | `tab` | [HistoryDrawer.tsx](../components/insights/HistoryDrawer.tsx) |
...
| `outside shipment created` | Parts leave the shop for a vendor and a slip is minted. **This replaces the un-instrumented Mark Sent Out** — the send now creates a document, so the object is the shipment rather than the operation. `shipment_index` is the number this feature was built to answer: all-ones means the quantity picker was ceremony and outside work really is whole-lot, while anything above one means partial sends are real and the old model was losing them. `surface` settles a claim [jobs.md](modules/jobs.md) makes with no evidence behind it — that the floor drives the send, not just the office. No raw quantity: `is_partial` is the shape, and a piece count is the customer's business data. | `surface`, `is_partial`, `shipment_index`, `has_due_back_date`, `has_instructions`, `has_ship_to_address` | [OperationsPanel.tsx](../components/jobs/OperationsPanel.tsx) |
| `outside shipment received` | Parts come back from a vendor and a receipt is recorded. `days_at_vendor_bucket` (`same_day`, `1_3d`, `4_7d`, `8_21d`, `over_21d`) is **the number that says whether outside processing is where jobs go to die**, and it is bucketed for the reason `elapsed_bucket` is — a raw per-slip duration answers nothing the bucket does not, and 21 days is already the shop-facing threshold the vendor page paints red. `was_backfilled` is the after-the-fact rate: nobody made a slip, the parts came back anyway, and the send was written alongside the receipt. If it stays high the send flow is not where the work happens and the slip is paperwork we invented. `short_closed` counts the times a shop wrote off what never came back — a vendor whose slips close short repeatedly is a vendor losing parts. | `surface`, `is_full`, `short_closed`, `was_backfilled`, `days_at_vendor_bucket` | [OperationsPanel.tsx](../components/jobs/OperationsPanel.tsx) |
| `outside shipment voided` | An outside-processing slip is voided from its preview. Low volume by design: this is the question of whether the document lifecycle earns its keep at all, and a shop voiding steadily is one whose send flow is producing slips it did not mean to. `was_received` separates a mistyped slip from a retraction that also has to unwind quantities, because the second reopens a step the floor already believed was finished. | `surface`, `was_received` | [OutsideShipmentPreviewDialog.tsx](../components/outsideShipments/OutsideShipmentPreviewDialog.tsx) |
| `lot certificate uploaded` | A mill certificate lands on a lot. `surface` says from where — `operator_receive` (the panel after stocking a part into a bin it was not already in), `operator` (the same panel after a top-up at a scanned bin), `office_receipt` (after an Add on the part page), `office_batch` (the block under the Storage bin form, after a delivery of several heats is put away at once), `office_lot` (attached later from the part's heats list, days after the truck). **`surface` is the number this feature exists to move.** The objection it answers was that scanning at the receiving bench is friction, so the ratio of dock surfaces to `office_lot` is what says whether certs are captured where the material lands or chased afterwards; `at_receipt` is that same question as one boolean, so a shop-level rate needs no breakdown. `file_kind` (`pdf`, `image`, `other`) says whether the shop's existing scan-to-PDF workflow feeds this or whether people photograph paper — the assumption the whole design rests on. `size_bucket` (`under_1mb`, `1_5mb`, `5_25mb`) is the upload's shape against the budget in `uploadTimeoutMs`. `is_replacement` says the lot already held a cert, which is what would justify a current-cert column if reissues turn out to be real. **Never a file name, never a heat number, never anything the certificate contains** | `surface`, `at_receipt`, `file_kind`, `size_bucket`, `is_replacement` | [LotCertificateControl.tsx](../components/inventory/LotCertificateControl.tsx) · [CertAfterReceiptPanel.tsx](../components/inventory/CertAfterReceiptPanel.tsx) |
| `lot certificate upload failed` | An upload is refused or fails with the stock already recorded. **This is the number that says what the never-blocks promise costs**: a receipt whose cert failed is material on the shelf with a document nobody has, and nothing on screen will chase it. `reason` is `rejected` (the client-side extension/size check, so nothing left the browser), `timeout` (`UploadTimeoutError`), or `failed` (everything the server refused). `attempt` counts tries within one panel, so a second failure is distinguishable from a first — and a population of only `attempt: 1` says the retry affordance is not being found | `surface`, `reason`, `attempt` | [LotCertificateControl.tsx](../components/inventory/LotCertificateControl.tsx) · [CertAfterReceiptPanel.tsx](../components/inventory/CertAfterReceiptPanel.tsx) |
| `storage view switched` | Someone switches between the Storage page's Inventory table and its Places board. **The one measurement that says whether the table earned its tab** — Inventory is the default, so a stream of switches TO `places` says the board is what people actually came for and the default is wrong, while a tab nobody ever opens is a tab to remove | `view` | [locations/page.tsx](../app/dashboard/[companyId]/inventory/locations/page.tsx) |
| `storage inventory loaded` | The Inventory table finishes its first read for a company. Reports the SHAPE of the shop's cost data — how many parts are on a shelf, and how many of them have no cost on file — which is the only way to know whether the gap disclosure is a footnote or the headline, and therefore whether the total means anything at all. `made_part_count` is counted apart from `no_cost_tier_part_count` on purpose: a made part has no purchase tier BY DESIGN, so folding the two together would report a design decision as a data-quality problem. `cost_visible` is whether the figures rendered (tenant flag AND company admin AND flags finished loading), which separates "this shop has no costs recorded" from "this viewer was not shown them". `truncated` means the read hit its ceiling and the total was withheld rather than printed short. **No amount, no total, no part identity is ever a property here** | `part_count`, `balance_count`, `no_cost_tier_part_count`, `made_part_count`, `below_min_part_count`, `cost_visible`, `truncated` | [StorageInventoryTable.tsx](../components/inventory/StorageInventoryTable.tsx) |
| `storage inventory filtered` | The Inventory table's search is used — `filter` is `search` (the only filter there is; a separate `where` select was removed on 2026-09-09 when the one box learned to match the place's full path). Says whether the table is read as a shop-wide total or narrowed to a rack, which is what decides whether place filtering ever deserves a control of its own again. Fires on the transition into a filtered state, not per keystroke | `filter`, `scope` | [StorageInventoryTable.tsx](../components/inventory/StorageInventoryTable.tsx) |
| `storage inventory gap opened` | Someone clicks the "no cost on file" line under the table to see which parts it means. **Separates noticing the gap from acting on it**, and the whole argument for putting the disclosure in words under the total rather than in a card above it rests on this being non-zero | `no_cost_tier_part_count` | [StorageInventoryTable.tsx](../components/inventory/StorageInventoryTable.tsx) |
| `marketing cta clicked` | Someone clicks a Request-access button on the public site. `location` is `hero`, `header` or `final`. **This is the number that says whether the page's length is costing conversions**: the middle 60% of the page had no CTA at all until 2026-09-11, so a `final` share that stays near zero means readers are deciding at the top and the sections below are decoration, while a healthy `final` share is what justifies the page carrying twenty feature names instead of three. Never the referrer, never a query string — those carry the prospect's own identity | `location` | [Hero.tsx](../components/marketing/Hero.tsx) · [MarketingHeader.tsx](../components/marketing/MarketingHeader.tsx) · [EmailCapture.tsx](../components/marketing/EmailCapture.tsx) |
| `waitlist submitted` | The request-access form is submitted and the row is written. `source` says which band converted (`landing_page`, `landing_footer_cta`, `coming_soon`, `invite_link`), and paired with `marketing cta clicked` it gives the open-to-submit rate — the only way to tell a page that does not persuade from a form that does not work. `shop_size` is the picker's own enum and is **the one property that says whether the page is reaching shops the product is built for**: the hero now qualifies for 3–30 people, so a stream of `50+ people` means the qualifier is not being read or not being believed. **Never the name, email or company** — the row in Supabase holds those, and the whole point of this table is that PostHog does not | `source`, `shop_size` | [EmailCapture.tsx](../components/marketing/EmailCapture.tsx) |
<!-- registry:end -->
