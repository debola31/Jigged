...
| `bom line charge basis set` | A BOM line is saved, or the bulk control sets every purchased material at once. **`rate_source` is the number this feature exists to learn** — whether shops override with per-material pricing tiers or just live on the shop-wide default. If it runs ~100% `company_default`, per-material tiers are not worth further investment in this niche | `basis`, `bulk`, `child_source`, `rate_source` | [PartBomPanel.tsx](../components/parts/PartBomPanel.tsx) |
| `material markup default set` | The shop-wide default material markup is saved in Settings. `cleared` distinguishes wiping it (back to requiring a tier per material) from changing the number | `had_previous`, `cleared` | [AppDefaultsCard.tsx](../components/settings/AppDefaultsCard.tsx) |
| `stock updated` | Stock is adjusted. `surface` says which UI — `parts` (office), `operator` (the four verbs on a part already in the bin), `operator_receive` (stocking a part into a bin it was not in yet); `location_id` is operator-only | `surface`, `action`, `part_id`, `quantity`, `unit`, `location_id` | [PartLocationActionModal.tsx](../components/parts/PartLocationActionModal.tsx) · [OperatorLocationActionModal.tsx](../components/operator/OperatorLocationActionModal.tsx) · [OperatorReceivePartModal.tsx](../components/operator/OperatorReceivePartModal.tsx) |
