...
| `storage inventory filtered` | A filter on the Inventory table is applied — `filter` is `search` or `where`. Says whether the table is used as a shop-wide total or as a per-place one, which is what decides whether the Where filter deserves to be a first-class control or a chip. Fires on the transition into a filtered state, not per keystroke | `filter`, `scope` | [StorageInventoryTable.tsx](../components/inventory/StorageInventoryTable.tsx) |
