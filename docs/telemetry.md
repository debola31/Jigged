...
| `running timer discarded` | The office discards a timer from the dashboard's Still-running list — the sibling of `time interval cancelled`, which is the operator discarding their own. **This is the forgotten-stop rate**, and it is the number the six-hour staleness threshold should be tuned against. Carries no duration and no operator: `get_open_intervals` returns neither, so there is nothing here that could resolve a machine to a person, and adding one would be the surveillance guardrail's exact failure mode | `surface` | [StillRunningCard.tsx](../components/dashboard/StillRunningCard.tsx) |
