...
| `parts created from drawings` | The import commits. `quotable_count` is the number that matters — a part is only quotable once it has priced work AND a markup, so the gap between `created_count` and `quotable_count` says how often the flow stops short of the thing it exists for. `updated_count` is separated from `created_count` on purpose: whether shops meet the same-number case at all is the question the identity guard was built for | `created_count`, `updated_count`, `failed_count`, `excluded_count`, `files_attached`, `with_operations`, `components_linked`, `quotable_count`, `used_ai`, `has_customer` | [parts/drawings/page.tsx](../app/dashboard/[companyId]/parts/drawings/page.tsx) |
