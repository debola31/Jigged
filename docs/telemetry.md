...
| `ai job settled` | That job reaches a terminal state, once per job. `phase` is what a local-inference rollout is judged on — the ratio of `offline` to `done` IS the question "is the box reliable enough", and nothing else answers it. `error_kind` separates a box being asleep from a model failing | `feature`, `phase`, `executor`, `model`, `error_kind`, `has_chart`, `tool_call_count` | [InsightsChat.tsx](../components/insights/InsightsChat.tsx) |
