...
| `drawing title blocks read` | The optional AI pass is pressed. `dropped_count` is the fidelity check refusing a value that was not on the drawing — it has never fired in measurement, so anything above zero means the model or the prompt changed | `asked_count`, `filled_count`, `skipped_count`, `failed_count`, `dropped_count` | [parts/drawings/page.tsx](../app/dashboard/[companyId]/parts/drawings/page.tsx) |
| `parts created from drawings` | The import commits. `quotable_count` is the number that matters — a part is only quotable once it has priced work AND a markup, so the gap between `created_count` and `quotable_count` says how often the flow stops short of the thing it exists for. `revived_count` and `updated_count` are separated from `created_count` on purpose: whether shops meet the archived-part and same-number cases at all is the question the identity guard was built for | `created_count`, `revived_count`, `updated_count`, `failed_count`, `excluded_count`, `files_attached`, `with_operations`, `components_linked`, `quotable_count`, `used_ai`, `has_customer` | [parts/drawings/page.tsx](../app/dashboard/[companyId]/parts/drawings/page.tsx) |
