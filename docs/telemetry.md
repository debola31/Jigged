...
| `terms prompt deferred` | An operator taps "Remind me later". `is_final_reminder` is a **boolean, never a tally** — the drop-off at the cap stays measurable without counting any individual's dismissals, which on an operator surface would be the surveillance shape in substance | `is_final_reminder` | [TermsGate.tsx](../components/legal/TermsGate.tsx) |
