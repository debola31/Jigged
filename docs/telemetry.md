...
| `completion voided` | The office undoes a recorded completion from the activity rail. Voiding shipped on the operation card's expand and was **never instrumented**; this is the write moving, which is the moment to close that gap. `capture_source` is the split worth having — the office voiding a row it keyed in itself is a typo, and the office voiding an `operator` row is the office overruling the floor. They are one button and two different things happening in the shop | `surface`, `capture_source` | [JobActivityRail.tsx](../components/jobs/activity/JobActivityRail.tsx) |
