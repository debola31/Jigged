...
| `parts created from drawings` | The import commits. `revived_count` and `updated_count` separated from `created_count` on purpose: whether shops meet the archived-part and same-number cases at all is the question the identity guard was built for | `created_count`, `revived_count`, `updated_count`, `failed_count`, `excluded_count`, `files_attached`, `used_ai`, `has_customer` | [parts/drawings/page.tsx](../app/dashboard/[companyId]/parts/drawings/page.tsx) |
