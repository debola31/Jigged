...
| `time interval closed` | A recorded interval ends. `close_reason` is the whole point of this row: `completed` is the happy path, `switched` never appears here (the chain writes it server-side, not a tap), and a rising `done_for_day` or `left_running` share says operators are stopping without finishing — which is the signal that the estimate, the routing or the material flow is wrong, not that the operator is | `close_reason`, `was_adjusted` | [OperatorIntervalContext.tsx](../components/operator/OperatorIntervalContext.tsx) |
