...
/** The three stored facts a backpay charge is derived from (billing.md §6.1). */
export interface BackpayFields {
...
/**
 * "$750.00 for 3 previously unbilled months (June – August 2026)" — the catch-up
 * Checkout will add to the first invoice.
 *
 * Returns `null` when there is nothing to say: no backpay configured, or it has already been paid
 * (`backpay_charged_at`), in which case advertising it again would be a lie. The month labels come
 * from the same two stored fields the backend derives its Stripe line items from, so this sentence
 * and the Stripe page cannot disagree — which is the whole reason the free-text description column
 * was dropped.
 */
export function backpaySummary(billing: BackpayFields | null): string | null {
  if (!billing || billing.backpay_charged_at) return null;
  const rate = billing.backpay_monthly_cents;
  const months = billing.backpay_months;
  const first = billing.backpay_first_month;
  if (!rate || !months || !first) return null;

  // Parse the parts by hand: `new Date('2026-06-01')` is UTC midnight, which renders as May in any
  // negative-offset timezone — and every shop on Jigged is in one.
  const [year, month] = first.split('-').map(Number);
  if (!year || !month) return null;
  const label = (offset: number) =>
    new Date(year, month - 1 + offset, 1).toLocaleDateString(undefined, {
      month: 'long',
      year: 'numeric',
    });

  const window = months === 1 ? label(0) : `${label(0)} – ${label(months - 1)}`;
  const total = ((rate * months) / 100).toLocaleString(undefined, {
    style: 'currency',
    currency: 'USD',
  });
  return `${total} for ${months} previously unbilled ${months === 1 ? 'month' : 'months'} (${window})`;
}
