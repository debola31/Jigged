...
function withinFileDuplicates(af: AnalyzedFile): Finding[] {
  ...
  return [{
    ...
    detail: `The repeats collapse into one record — they won't come in twice.`,
    count: affected,
  ...
  for (const { childEntity, childField, parentEntity, parentField } of REFERENTIAL_LINKS) {
    ...
    if (orphanCount) {
      ...
      out.push({
        ...
        detail: `Used by ${orphanCount.toLocaleString()} ${childLabel} row(s).`,
        count: orphanCount,
...
function costCoverage(files: AnalyzedFile[]): Finding[] {
  ...
  for (const af of filesOf(files, 'parts')) {
    ...
    out.push({
      ...
      detail: `${missing.toLocaleString()} of ${total.toLocaleString()} parts have no cost — you can add prices in Jigged later.`,
      count: missing,
      examples: [],
      source_files: [af.filename],
      verified: true,
  ...
  for (const entity of ['parts', 'vendors'] as EntityType[]) {
    ...
    for (const af of filesOf(files, entity)) {
      ...
      out.push({
        ...
        detail: `They'd import as separate ${entityLabel(entity)} unless you combine them.`,
