  ...
  try {
    ...
    if (req.method === 'POST' && pathParts.length === 1) {
      ...
      if (existingUser) {
        ...
        const { data: existingAccess } = await supabase.from('user_company_access').select('id').eq('user_id', existingUser.id).eq('company_id', company_id).single();
        if (existingAccess) {
          return errorResponse('This user already has access to this company', 400);
        }
        // Create user_company_access for existing user
        const { data: newAccess, error: accessError } = await supabase.from('user_company_access').insert({
      ...
      const { data: accessRecord, error: accessError } = await supabase.from('user_company_access').insert({
        user_id: userId,
        company_id,
        role,
        name
