-- 20260514_unify_bom_cost_rollup.sql
--
-- Collapse three coexisting cost sources (`parts.cost_per_unit` snapshot,
-- `part_pricing_tiers.base_cost_per_unit` stored derivation,
-- `part_procurement_tiers.cost_per_unit` vendor truth) into a single live
-- function, `compute_part_cost_at_qty(part_id, qty)`. The function walks the
-- BOM recursively, picks each leaf's tier at the cumulative qty cascaded down,
-- and rolls labor + setup + materials up to the root.
--
-- Setup amortization (called out per plan):
--   Setup is amortized over `p_qty` at every node. When the root is invoked
--   at p_qty = parent_order_qty, sub-assemblies are invoked recursively at
--   `p_qty = parent_p_qty × bom_qty_in_child_primary_unit`. A sub-assembly's
--   per-parent-unit setup contribution works out to:
--     bom_qty × (sub_setup / (parent_p_qty × bom_qty)) = sub_setup / parent_p_qty
--   i.e. one sub-assembly setup spread across the whole parent batch run,
--   independent of how many sub-units that parent consumes.
--
-- Bought-part tier preference: `vendor_id IS NOT NULL` wins over NULL-vendor
-- tiers when both match the requested qty. NULL-vendor tiers act as the
-- import / "internal estimate" fallback row (no sentinel vendor required).
--
-- Diamond BOMs: the function is STABLE so Postgres may memoize repeat
-- `(part_id, p_qty)` tuples within a single query. Even without memoization,
-- correctness holds — each branch computes independently.
--
-- Depth bound: 50, matching `enforce_no_bom_cycles` trigger.

BEGIN;

-- =========================================================================
-- 1. Rewrite get_procurement_cost: prefer vendor-specific tiers, drop the
--    parts.cost_per_unit fallback. Returns zero rows when no tier matches.
-- =========================================================================
...
-- =========================================================================
-- 2. compute_part_cost_at_qty — canonical cost lookup.
-- =========================================================================
...
-- =========================================================================
-- 3. compute_part_cost_explain — same cost number plus a skinny list of the
--    bought leaves whose tier lookup returned NULL.
-- =========================================================================
...
-- =========================================================================
-- 4. Rewrite seed_demo_data so it stops writing parts.cost_per_unit. For
--    bought parts whose template carries a cost_per_unit, emit a NULL-vendor
--    procurement tier at min_quantity=1 — no sentinel vendor row needed. Made
--    parts' template cost_per_unit values are ignored (compute_part_cost_at_qty
...
-- =========================================================================
-- 5. Drop the legacy snapshot column, helper function, and base_cost_per_unit.
-- =========================================================================

DROP FUNCTION IF EXISTS public.recalculate_part_cost(uuid);

ALTER TABLE public.parts
    DROP COLUMN IF EXISTS cost_per_unit;

ALTER TABLE public.parts
    DROP COLUMN IF EXISTS cost_recalculated_at;

ALTER TABLE public.part_pricing_tiers
    DROP COLUMN IF EXISTS base_cost_per_unit;

COMMENT ON FUNCTION public.compute_part_cost_at_qty(uuid, numeric) IS
  'Canonical per-unit cost lookup. Walks the BOM recursively, picking each '
  'leaf''s tier at the cumulative qty cascaded down. Setup is amortized over '
  'p_qty at every node — see the migration header for the sub-assembly math. '
  'Returns NULL when any bought leaf has no matching procurement tier. Raises '
  'when a made part has a routing op with no labor rate or no external '
  'pricing. Depth-bounded at 50 to match enforce_no_bom_cycles.';

COMMENT ON FUNCTION public.compute_part_cost_explain(uuid, numeric) IS
  'Same cost as compute_part_cost_at_qty, plus a jsonb array of the bought '
  'leaves whose tier lookup returned NULL at the cascaded qty. UI surfaces '
  'these as actionable tooltips. Made parts with empty BOM/routing are not '
  '"missing" — they cost 0.';
