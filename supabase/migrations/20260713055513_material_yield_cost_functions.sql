...
-- Priceability / missing-leaf walk. Mirror the batch-pin + ceiling logic so the
-- cascaded qty a bought leaf is tier-checked at matches what the cost function
-- actually consumes. Pinning RESETS a made child's cascaded qty to its batch
-- qty for its whole subtree (a bought leaf under a pinned child is required at
-- the batch qty, e.g. 25, not the cascaded fraction) — this can flip a
-- tier-coverage verdict, which is the correct, honest result.
