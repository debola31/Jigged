-- Floor procurement pricing to the lowest tier when the (cascaded) quantity is
-- below every break — you can't buy less than the smallest pack, so you pay its
-- per-unit price rather than getting NO price.
--
-- Before: compute_part_cost_at_qty's bought branch matched only tiers with
-- min_quantity <= p_qty and returned NULL when none did. So a part whose BOM
-- cascaded a bought leaf to a sub-min qty (e.g. a made part costed at a tier of
-- 0.4, or a fractional/yield consumption) had NO base cost — the Pricing card's
-- Base/unit + Unit price went blank. This mirrors the markup resolver, which
-- already floors to the lowest tier below the minimum (`below_min`).
--
-- Re-issued here (last definition wins) so both functions stay consistent:
--   * compute_part_cost_at_qty: bought branch floors to the lowest-min tier.
--   * compute_part_cost_explain: a bought leaf is "missing" only if it has NO
--     procurement tier at all (any tier now makes it priceable at any qty).
-- The batch-pin + whole-unit-ceiling logic and the NULL-markup priceability
-- guard are carried forward unchanged.
...
BEGIN
    ...
    -- A bought leaf is "missing" only if it has NO procurement tier for its
    -- preferred vendor at all. Because the cost function now FLOORS to the
    -- lowest tier below the minimum, any tier makes the leaf priceable at any
    -- qty — so the qty-vs-min_quantity check is dropped (it would have flagged a
    -- leaf as unpriceable at a cascaded sub-min qty while the cost function
    -- actually prices it).
    leaves AS (
