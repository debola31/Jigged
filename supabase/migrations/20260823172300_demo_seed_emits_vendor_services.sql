-- Demo companies emit vendor_services, not kind='external' work centres.
--
-- seed_demo_data cloned every template work_centers entry into work_centers,
-- kind and all. After the split that INSERT names two columns that no longer
-- exist, so creating or resetting a demo company would fail outright.
--
-- Rebuilt from its NEWEST definition (20260819012414_drop_part_is_stocked),
-- never from the migration that created it — rebuilding from a creating
-- migration silently reverts every fix applied since, which this repo has been
-- bitten by more than once.
--
-- The TEMPLATE JSON is deliberately untouched. An entry carrying
-- kind='external' and a vendor_ref has always described a vendor's service
-- rather than a station; only the destination table changed. Rewriting the
-- template rows would be churn on data that was already saying the right thing.
...
-- ── reset_demo_company learns about the new table ───────────────────────────
-- Rebuilt from its NEWEST definition (20260811010948_drop_quote_cost_snapshots).
-- The delete order here is load-bearing: it walks children before parents, and
-- vendor_services sits between work_centers and vendors. Without this row a
-- demo reset fails with a 23503 on vendor_services_vendor_id_fkey — which is
-- exactly what the demo-lifecycle tests caught.
