-- The Playbook sheet learns how long a clip is.
--
-- WHY A 190-LINE RE-EMISSION FOR ONE FIELD. This RPC is the FOURTH read path for
-- note media and the only one that is not a PostgREST select: it hand-builds the
-- media array with jsonb_build_object, and utils/operatorAccess.ts passes that
-- jsonb straight through as JobNoteMedia[] with no mapper. So a column added to
-- the table and to the three select strings still arrives `undefined` here, and
-- the type would be asserting something untrue on exactly one surface. Two notes
-- showing the same clip would disagree about whether it has a duration.
--
-- `kind` was already emitted, so the play badge and playback on the Playbook sheet
-- needed nothing. Only the duration chip did.
--
-- `note_id` goes in at the same time. JobNoteMedia has always declared it
-- required and this path has never emitted it -- an existing quiet lie, cheap to
-- close while the function is open anyway.
--
-- THE COMMENT AND THE GRANTS ARE RE-ISSUED BELOW, and that is not boilerplate. A
-- DROP FUNCTION destroys both, which is precisely how 20260729133520 shipped this
-- RPC anon-executable for three days with no visible symptom -- the reason
-- playbook_rpc_execute_leaks() exists as a CI guard. Everything below the body is
-- carried forward verbatim from 20260801012258, the newest definition.
...
AS $$
  ...
  LEFT JOIN LATERAL (
    SELECT jsonb_agg(jsonb_build_object(
             ...
             -- Never emitted, though JobNoteMedia has always declared it required.
             -- Harmless until now only because nothing read it off this path.
             'note_id', md.note_id
