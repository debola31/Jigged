-- ============================================================================
-- A demo template places its stock, or says so
-- ============================================================================
-- 20260906182638 removed the `Unassigned` bucket and made
-- `enforce_tracked_part_quantity` refuse a part created carrying a quantity.
-- `seed_demo_data` still had the arm that produced exactly that: a template item
-- with no `stock` array passed its `quantity` onto the part row, relying on
-- `seed_new_part_balance` to park the number in the bucket. Both of those are
-- gone, so the arm could no longer do anything but fail -- with a trigger error
-- about a part the template author did not write by hand.
--
-- The active template (v4) places stock for all 45 of its parts, so demo mode
-- works today and this changes nothing for it. The INACTIVE v2 has four items
-- that carry a bare quantity, which is what makes this worth closing rather than
-- leaving: reactivating an older template version is an ordinary rollback, and
-- it would have failed obscurely.
--
-- Recreated from the LIVE definition (`pg_get_functiondef`) with the comment
-- corrected, the CASE replaced by a hard 0, and a guard added -- not from the
-- migration that created it, which is how a body silently reverts (CLAUDE.md;
-- five occurrences in this repo).
-- ============================================================================
...
BEGIN
    ...
    IF v_template->'parts' IS NOT NULL THEN
        FOR v_item IN SELECT * FROM jsonb_array_elements(v_template->'parts') LOOP
            ...
            -- quantity is DERIVED from part_location_stock by trigger, always.
            --
            -- There used to be a second arm: a template item with no `stock`
            -- array passed its `quantity` straight onto the part row, and
            -- `seed_new_part_balance` parked that number at the company's
            -- `Unassigned` bucket. Both the trigger and the bucket went in
            -- 20260906182638, and `enforce_tracked_part_quantity` now REFUSES a
            -- part created carrying a quantity -- so that arm could no longer
            -- do anything but fail.
            --
            -- It fails LOUDLY here instead, naming the item, because the
            -- alternative is a template author reading a trigger error about a
            -- part they did not write. Every item in the active template (v4)
            -- already places its stock; the inactive v2 has four that do not,
            -- which is exactly the reactivation this guards.
            IF v_item->'stock' IS NULL
               AND COALESCE((v_item->>'quantity')::numeric, 0) <> 0 THEN
                RAISE EXCEPTION 'Demo template item % carries a quantity but no stock array; stock has to name a location since 20260906182638',
                    v_item->>'part_name'
...
BEGIN
    IF (SELECT count(*) FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
         WHERE n.nspname = 'public' AND p.proname = 'seed_demo_data') <> 1 THEN
        RAISE EXCEPTION 'seed_demo_data must have exactly one overload';
    END IF;

    -- The arm that could only fail is really gone. Asserted on the EXPRESSION rather
    -- than on the trigger's name: the comment above the insert names that trigger
    -- deliberately, so someone reading the body learns what used to happen -- and a
    -- guard matching the name would fail on its own explanation.
    IF (SELECT prosrc FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
         WHERE n.nspname = 'public' AND p.proname = 'seed_demo_data')
       LIKE '%ELSE COALESCE((v_item->>''quantity'')::numeric, 0) END%' THEN
        RAISE EXCEPTION 'seed_demo_data still passes a template quantity onto the part row';
    END IF;
END $guard$;
