 ...
 * constraint pinning it to QuoteStatus values (`quotes_status_check`, in
 * the baseline migration); the generated Database type only sees `string`.
 * Centralized here so the assertion is documented once instead of scattered.
 */
...
export async function createQuote(
  ...
  const { data: quote, error } = await supabase
    ...
    .insert({
      ...
      // quote_number is NOT NULL but the set_quote_number trigger (defined in
      // the baseline, reworked by 20260621213555_unify_order_numbering) fills
      // it from generate_quote_number() when the value is '' or NULL. Sending ''
